...
class QFictitiousPlayer:
    ...
    def __init__(self, actions=2):
        self.epsilon = 1
        self.visits = np.zeros(actions,actions)
        self.q_table = np.zeros(actions,actions) #[3][3]
    ...
    def get_probability_action(self,action,opp_action,k):
        temp = 0.1
        return (np.exp(self.q_table[action][opp_action]/temp))/(np.exp(self.q_table[0][0]/temp) + np.exp(self.q_table[1][0]/temp) + np.exp(self.q_table[2][0]/temp)
